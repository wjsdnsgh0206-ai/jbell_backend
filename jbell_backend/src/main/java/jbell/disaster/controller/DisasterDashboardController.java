...
public class DisasterDashboardController {
	...
	public ResponseEntity<?> updateMessageVisibility(@RequestBody DisasterBatchRequest request) {

		log.info("노출 변경 요청 ids={}, visibleYn={}", request.getIds(), request.getVisibleYn());

		boolean result = disasterService.updateDisasterVisibility(request.getVisibleYn(), request.getIds());
	...
	public ResponseEntity<String> updateWeather(@PathVariable("key") String key,
			@RequestBody PredictionInfoResponse dto) {
	...
	public ResponseEntity<String> updateWeatherVisibility(@RequestBody DisasterBatchRequest request) {
		...
		log.info("기상특보 노출 변경 요청 ids={}, visibleYn={}", request.getIds(), request.getVisibleYn());

		// DisasterBatchRequest.getIds() → List<Long> 라면 변환
		List<String> ids = request.getIds().stream().map(String::valueOf).toList();

		boolean result = disasterService.updateWeatherVisibility(ids, request.getVisibleYn());
...
//        boolean updateWeatherVisibility( List<String> ids, String visibleYn);
	...
	/**
	 * 산사태 정보 api 요청 주소 /api/disaster/dashboard/predictionInfo 요청정보 json { "pageNo"
	 * : "1", "numOfRows" : "10", "type" : "xml", // 기본값 json "sgg" : "전라북도" }
	 */
	@PostMapping("/predictionInfo")
	public Mono<ApiResponse<PublicDataResponse<PredictionInfoResponse>>> getLandslidePredictionInfo(
			@Valid @RequestBody DisasterExternApiRequest request) {
		...
		return publicDataService.getLandslidePredictionInfo(request, PredictionInfoResponse.class)
				...
				.onErrorResume(e -> {
					...
					return Mono.just(ApiResponse.error(ErrorCode.EXTERNAL_API_ERROR.code(),
							ErrorCode.EXTERNAL_API_ERROR.message()));
				}).defaultIfEmpty(ApiResponse.error(ErrorCode.NOT_FOUND.code(), ErrorCode.NOT_FOUND.message()));
	}

	/**
	 * 통합정보(날씨 정보, 지진특보, 진도정보)api 요청 주소 /api/disaster/dashboard/integrationInfo 요청정보
	 * json { "weather": { "lat" : "35.8350848", "lon" : "127.123456", "units" :
	 * "metric", "lang" : "kr" }, "earthquake": { "orderTy" : "xml", "eqArCd" :
	 * "A16" } }
	 */
	@PostMapping("/integrationInfo")
	public Mono<ApiResponse<JsonNode>> getIntegrationInfo(@RequestBody JsonNode request) {

		return integrationService.getIntegrationInfo(request).map(ApiResponse::success) // 성공 시 표준 응답으로 래핑
				.onErrorResume(e -> {
					log.error("Error occurred: {}", e.getMessage());
					return Mono.just(ApiResponse.error(ErrorCode.EXTERNAL_API_ERROR.code(),
							ErrorCode.EXTERNAL_API_ERROR.message()));
	...
	// =============================================================
	/**
	 * 재난문자이력 api 요청 주소 /api/disaster/dashboard/disasterMessageInfo 요청정보 json {
	 * "pageNo" : "1", "numOfRows" : "30", "type" : "json", "crtDt" : "rgnNm" : "전북"
	 * }
	 */

	// =============================================================
	/**
	 * 재난문자이력 api 요청 및 저장 주소 /api/disaster/dashboard/disasterMessageInfo *
	 * 이제 @RequestBody가 없으므로 Postman의 Params 탭에 값을 넣어 호출해!
	 */
	@PostMapping("/disasterMessageInfo")
	public Mono<ApiResponse<SafetyDataResponse<PredictionInfoResponse>>> getDisasterMessageInfo(
			@Valid DisasterExternApiRequest request) { // @RequestBody를 제거해서 이제 URL 파라미터(?pageNo=1&...)를 자동으로 인식함
		log.info("재난문자 수집 요청 파라미터: {}", request);

		return publicDataService.getAndSaveDisasterMessages(request).map(ApiResponse::success).onErrorResume(e -> {
	...
	// =============================================================
	/**
	 * 기상 특보 api 요청 및 저장 주소 /api/disaster/dashboard/weatherWarningInfo 요청정보 json {
	 * "pageNo" : "1", "numOfRows" : "50", "inqDt" : "20260128" }
	 */
	@PostMapping("/weatherWarningInfo")
	public Mono<ApiResponse<SafetyDataResponse<PredictionInfoResponse>>> collectWeatherWarnings(
			@Valid @RequestBody DisasterExternApiRequest request) {
		return publicDataService.getAndSaveWeatherWarnings(request).map(ApiResponse::success).onErrorResume(e -> {
