...
public class PublicDataService {
    ...
    public Mono<SafetyDataResponse<PredictionInfoResponse>> getAndSaveWeatherWarnings(DisasterExternApiRequest request) {
        ...
        // 2. DB에서 현재 가장 최신 발표 시각(PRSNTN_TM) 가져오기
        String latestPrsntnTm = weatherWarningMapper.selectLatestPrsntnTm();
        log.info("현재 DB 내 최신 발표 시각: {}", latestPrsntnTm);

        // 3. 7일치 반복 수집
        return Flux.range(0, 7)
                .concatMap(day -> { // 순서대로 처리하기 위해 concatMap 사용
                    String targetDate = java.time.LocalDate.now().minusDays(day)
                            .format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd"));
                    
                    DisasterExternApiRequest dailyReq = new DisasterExternApiRequest();
                    dailyReq.setInqDt(targetDate);
                    dailyReq.setNumOfRows(100); 
                    dailyReq.setPageNo(1);

                    return getWeatherWarningInfo(dailyReq, PredictionInfoResponse.class)
                            .delayElement(Duration.ofMillis(300)) // 서버 부하 방지
                            .doOnNext(response -> {
                                List<PredictionInfoResponse> body = response.getBody();
                                if (body != null && !body.isEmpty()) {
                                    
                                    // ⭐ [핵심 로직] DB의 최신 시각보다 더 나중인 데이터만 필터링
                                    List<PredictionInfoResponse> newItems = body.stream()
                                            .filter(item -> {
                                                if (latestPrsntnTm == null) return true; // DB가 비어있으면 전체 저장
                                                if (item.getPrsntnTm() == null) return false;
                                                // 문자열 비교 (202601281400 > 202601281300 식의 비교)
                                                return item.getPrsntnTm().compareTo(latestPrsntnTm) > 0;
                                            })
                                            .toList();

                                    if (!newItems.isEmpty()) {
                                        weatherWarningMapper.insertWeatherWarnings(newItems);
                                        log.info("{} 일자 신규 기상 특보 {}건 저장 완료", targetDate, newItems.size());
                                    } else {
                                        log.info("{} 일자에는 추가할 최신 데이터가 없습니다.", targetDate);
                                    }
                                }
                            })
                            .onErrorResume(e -> {
                                log.error("{} 일자 기상청 수집 실패: {}", targetDate, e.getMessage());
                                return Mono.empty();
                            });
                })
